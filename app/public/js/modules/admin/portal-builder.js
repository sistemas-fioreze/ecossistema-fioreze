...
export function createVisualPortalBuilder({ onSaved = () => {} } = {}) {
  ...
  function renderBlockPreview(block) {
    ...
    if (block.type === "feature-grid") return `<div class="vp-preview-inner vp-preview-grid ${content.layout === "overlay" ? "is-overlay" : ""}" style="--vp-card-copy-bg:${escapeAttr(content.text_background_color)}">${content.items.map((item) => `<article>${mediaThumbnail(item.media_asset_id)}<div><h3>${escapeHtml(item.title || "Novo destaque")}</h3>${previewParagraphs(item.text)}${previewButton(item.button_text, item.button_url, "ghost")}</div></article>`).join("") || mediaPlaceholder("Adicione itens à grade")}</div>`;
...
function createBlock(type) {
  ...
  const content = {
    ...
    "feature-grid": { layout: "stacked", text_background_color: "#ffffffee", items: [{ title: "Novo destaque", text: "Descreva este conteúdo.", media_asset_id: "", button_text: "", button_url: "" }] },
