export function renderBilling({ outlet, orders, billing }) {
  ...
  outlet.innerHTML = `
    ...
    <section class="rs-report-grid">
      <article class="rs-panel"><span class="rs-muted">Total visível</span><strong>${formatMoney(total)}</strong></article>
      <article class="rs-panel"><span class="rs-muted">Pedidos entregues</span><strong>${summary.completed_orders ?? 0}</strong></article>
      <article class="rs-panel"><span class="rs-muted">Ticket médio</span><strong>${formatMoney(summary.average_ticket_cents)}</strong></article>
    </section>
    <section class="rs-panel"><div class="rs-empty">${escapeHtml(billing?.exports?.message || "Exportações ainda não habilitadas.")}</div></section>
  `;
}
