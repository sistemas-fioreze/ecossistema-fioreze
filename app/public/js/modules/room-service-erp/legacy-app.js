...
function installOperationalModals() {
  ...
  byId("catalogImageSearch").addEventListener("input", renderCatalogImagePicker);
  byId("catalogMediaFile").addEventListener("change", () => {
    const file = byId("catalogMediaFile").files?.[0];
    byId("catalogMediaFileName").textContent = file?.name || "Nenhum arquivo selecionado";
  });
...
function openCatalogItemModal(item = null) {
  ...
  byId("catalogImageSearch").value = "";
  byId("catalogMediaFile").value = "";
  byId("catalogMediaFileName").textContent = "Nenhum arquivo selecionado";
...
function renderCatalogImagePicker() {
  ...
  const picker = byId("catalogImagePicker");
  const scrollTop = picker.scrollTop;
  const query = normalize(byId("catalogImageSearch").value);
  const assets = state.media.filter((asset) => normalize([
    asset.alt_text,
    asset.file_name,
    asset.filename,
    asset.object_key,
  ].filter(Boolean).join(" ")).includes(query));
  const imageOption = (asset) => {
    const mediaId = asset?.id || "";
    const isSelected = selected === mediaId;
    const label = catalogMediaLabel(asset);
    const preview = asset
      ? `<span class="erp-image-option-preview"><img src="${escapeAttr(asset.public_url)}" alt="" loading="lazy"></span>`
      : `<span class="erp-image-option-preview erp-image-option-empty">${imagePlaceholderIcon()}</span>`;
    return `<button type="button" class="erp-image-option ${isSelected ? "selected" : ""}" data-media-id="${escapeAttr(mediaId)}" role="option" aria-selected="${isSelected}" title="${escapeAttr(label)}">${preview}<span class="erp-image-option-copy"><span class="erp-image-option-label">${escapeHtml(label)}</span><span class="erp-image-option-state">${isSelected ? "Selecionada" : "Selecionar"}</span></span></button>`;
  };
  picker.innerHTML = imageOption(null) + assets.map(imageOption).join("") + (query && !assets.length ? `<p class="erp-image-picker-empty">Nenhuma imagem encontrada.</p>` : "");
  byId("catalogImageSummary").textContent = query ? `${assets.length} resultado${assets.length === 1 ? "" : "s"}` : `${state.media.length} imagens`;
  picker.scrollTop = scrollTop;
}

function catalogMediaLabel(asset) {
  if (!asset) return "Sem imagem";
  const objectName = String(asset.object_key || "").split("/").pop();
  return String(asset.alt_text || asset.file_name || asset.filename || objectName || "Imagem sem nome").trim();
}
