...
export function createShell({ session, preferences, onNavigate, onHotelChange, onPreferenceChange }) {
  ...
  function renderNav() {
    ...
    els.nav.innerHTML = NAV_ITEMS.filter((item) => item.sidebar !== false).map((item) => {
      ...
      return `<button type="button" data-route="${item.key}" aria-current="${item.key === activeRoute ? "page" : "false"}"><span class="erp-nav-icon" aria-hidden="true">${navIcon(item.key)}</span><span>${item.label}</span></button>`;
    }).join("");
...
function navIcon(route) {
  ...
  return icons[route] ? iconMarkup(icons[route]) : "";
}
