...
test("eventos e informacoes usam datas normalizadas e hotel autorizado", async () => {
  ...
  const event = await createPortalEvent({
    request: request({ hotel_id: "muller-fioreze", title: "Encontro cultural", summary: "Programacao especial", starts_at: "2026-07-20T18:00:00-03:00", ends_at: "2026-07-20T20:00:00-03:00", timezone: "America/Sao_Paulo", status: "published" }),
    env,
    session: session(),
  });
...
class ContentStatement {
  ...
  async all() {
    ...
    if (this.sql.includes("FROM events") && this.sql.includes("WHERE hotel_id = ?")) return { results: data.events.filter((row) => row.hotel_id === this.params[0]) };
  ...
  async first() {
    ...
    if (this.sql.includes("FROM events WHERE id")) return data.events.find((row) => row.id === this.params[0]) || null;
  ...
  execute() {
    ...
    if (this.sql.startsWith("INSERT INTO portal_pages")) {
      ...
      data.events.push({ id: p[0], hotel_id: p[1], title: p[2], summary: p[3], starts_at: p[4], ends_at: p[5], timezone: p[6], status: p[7], created_at: p[8], updated_at: p[9] });
