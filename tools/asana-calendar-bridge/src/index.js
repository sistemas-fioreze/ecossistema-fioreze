...
export default {
  ...
  async scheduled(_controller, env, ctx) {
    ctx.waitUntil(
      syncBridge(env)
        .then((result) => console.log("Asana Google sync complete", JSON.stringify(result)))
        .catch((error) => console.error("Asana Google sync failed", error?.stack || String(error))),
    );
  },
};
...
async function syncHybrid(env) {
  ...
  // First resolve every currently-incomplete Asana task against its Google Task.
  for (const task of asanaTasks) {
    if (!hasSchedule(task)) continue;
  ...
  // Pending Google Tasks missing from the incomplete Asana result need one detail lookup:
  // completed in Asana -> complete Google; reassigned/date removed/timed/deleted -> remove Google.
...
async function fetchAllIncompleteAsanaTasks(env) {
  ...
  do {
    const params = new URLSearchParams({
      ...
      limit: "100",
      opt_fields: [
        "gid", "name", "notes", "completed", "due_on", "due_at", "start_on", "start_at",
        "modified_at", "permalink_url", "memberships.project.name", "assignee.gid",
      ].join(","),
    });
...
function createGoogleTask(taskListId, body, accessToken) {
  return googleTasksRequest(`/lists/${encodeURIComponent(taskListId)}/tasks`, accessToken, { method: "POST", body });
}
...
async function syncCalendarEvents(env, desiredTasks, accessToken) {
  ...
  const stats = { existingEvents: existingEvents.length, calendarCreated: 0, calendarUpdated: 0, calendarDeleted: 0, calendarUnchanged: 0 };

  await mapLimit(desiredTasks, 8, async (task) => {
    const desired = buildGoogleEvent(task, env);
    const existing = eventsByTask.get(task.gid);
    if (!existing) {
      await googleCalendarRequest(`/calendars/${encodeURIComponent(calendarId)}/events?sendUpdates=none`, accessToken, { method: "POST", body: desired });
      stats.calendarCreated += 1;
    } else if (eventMatches(existing, desired)) {
      stats.calendarUnchanged += 1;
    } else {
      await googleCalendarRequest(`/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(existing.id)}?sendUpdates=none`, accessToken, { method: "PATCH", body: desired });
  ...
  await mapLimit([...stale, ...duplicates], 8, async (event) => {
    await googleCalendarRequest(`/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(event.id)}?sendUpdates=none`, accessToken, { method: "DELETE" });
...
function buildGoogleEvent(task, env) {
  ...
  if (!task.start_at || end.getTime() <= start.getTime()) end = new Date(start.getTime() + durationMinutes * 60_000);
  ...
  return {
    ...
    extendedProperties: { private: { asanaBridge: bridgeId(env), asanaTaskGid: task.gid } },
  };
}

function eventMatches(existing, desired) {
  const a = {
    summary: existing.summary || "", description: existing.description || "", transparency: existing.transparency || "opaque",
    start: normalizeEventBoundary(existing.start), end: normalizeEventBoundary(existing.end),
    asanaBridge: existing.extendedProperties?.private?.asanaBridge || "", asanaTaskGid: existing.extendedProperties?.private?.asanaTaskGid || "",
  };
  const b = {
    summary: desired.summary || "", description: desired.description || "", transparency: desired.transparency || "opaque",
    start: normalizeEventBoundary(desired.start), end: normalizeEventBoundary(desired.end),
    asanaBridge: desired.extendedProperties?.private?.asanaBridge || "", asanaTaskGid: desired.extendedProperties?.private?.asanaTaskGid || "",
  };
...
async function listManagedGoogleEvents(calendarId, accessToken, id) {
  ...
  do {
    ...
    if (pageToken) params.set("pageToken", pageToken);
    const payload = await googleCalendarRequest(`/calendars/${encodeURIComponent(calendarId)}/events?${params}`, accessToken);
...
async function getServiceAccountAccessToken(serviceAccountJson) {
  let credentials;
  try { credentials = JSON.parse(serviceAccountJson); } catch { throw new Error("GOOGLE_SERVICE_ACCOUNT_JSON is not valid JSON"); }
  if (!credentials.client_email || !credentials.private_key) throw new Error("GOOGLE_SERVICE_ACCOUNT_JSON must contain client_email and private_key");

  const now = Math.floor(Date.now() / 1000);
  const header = base64UrlEncode(JSON.stringify({ alg: "RS256", typ: "JWT" }));
  const claims = base64UrlEncode(JSON.stringify({ iss: credentials.client_email, scope: GOOGLE_CALENDAR_SCOPE, aud: GOOGLE_TOKEN_URL, iat: now, exp: now + 3600 }));
  const unsignedToken = `${header}.${claims}`;
  const privateKey = await importPkcs8PrivateKey(credentials.private_key);
  const signature = await crypto.subtle.sign({ name: "RSASSA-PKCS1-v1_5" }, privateKey, new TextEncoder().encode(unsignedToken));
  const assertion = `${unsignedToken}.${base64UrlEncode(new Uint8Array(signature))}`;
  const payload = await tokenRequest({ grant_type: "urn:ietf:params:oauth:grant-type:jwt-bearer", assertion }, "Google service account OAuth");
  if (!payload.access_token) throw new Error("Google service account OAuth did not return an access_token");
...
async function startGoogleTasksOAuth(request, env) {
  if (!env.GOOGLE_OAUTH_CLIENT_ID || !env.GOOGLE_OAUTH_CLIENT_SECRET) {
    return jsonResponse({ ok: false, error: "Configure GOOGLE_OAUTH_CLIENT_ID and GOOGLE_OAUTH_CLIENT_SECRET first." }, 503);
  }
  const url = new URL(request.url);
...
async function finishGoogleTasksOAuth(request, env) {
  if (!env.GOOGLE_OAUTH_CLIENT_ID || !env.GOOGLE_OAUTH_CLIENT_SECRET) {
    return htmlResponse("<h1>OAuth não configurado</h1><p>Faltam GOOGLE_OAUTH_CLIENT_ID / GOOGLE_OAUTH_CLIENT_SECRET.</p>", 503);
  }
  const url = new URL(request.url);
  if (url.searchParams.get("error")) return htmlResponse(`<h1>Autorização cancelada</h1><pre>${escapeHtml(url.searchParams.get("error"))}</pre>`, 400);
  ...
  if (!payload.refresh_token) {
    return htmlResponse("<h1>Sem refresh token</h1><p>Abra /oauth/start novamente. O Worker já solicita prompt=consent.</p>", 503);
  }
...
async function verifyOAuthState(state, secret) {
  ...
  let payload;
  try { payload = new TextDecoder().decode(base64UrlDecode(encoded)); } catch { return false; }
